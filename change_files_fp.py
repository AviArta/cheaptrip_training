...
def search_change_files(directory: str, first_data: str, new_data: str, key: str='images'):  
    ...
    for one_file in pathlib.Path(directory).rglob('*.json'):
        try:
            lines = read_file(one_file)
            counter_files += 1
            result_lines = modify_data(lines, first_data, new_data, key)
...
def modify_data(lines: list, first_data: str, new_data: str, key: str, counter_change_files: int=0) -> list:
    ''' The function creats the dictionary with modified value-substrings by key. 
    It accepts parameters: list, first substring, new substring, key for changing value,
